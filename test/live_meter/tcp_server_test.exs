defmodule LiveMeter.TCPServerTest do
  ...
  test "PubSub telegrams are sent to clients connected in parallel" do
    ...
    start_supervised!({TCPServer, name: name, port: 0, pubsub_topic: topic})
    ...
    for client <- clients do
      assert {:ok, ^telegram} = :gen_tcp.recv(client, 0, 1_000)
    end
  ...
  test "closing one client does not prevent other clients from receiving later broadcasts" do
    ...
    assert {:ok, received} = :gen_tcp.recv(active_client, 0, 1_000)
    assert received == first <> second or received == first

    if received == first do
      assert {:ok, ^second} = :gen_tcp.recv(active_client, 0, 1_000)
    end
  ...
  test "smart meter broadcasts generated telegrams through TCP server" do
    ...
    start_supervised!({TCPServer, name: tcp_name, port: 0, pubsub_topic: topic})
    ...
    assert {:ok, telegram_string} = :gen_tcp.recv(client, 0, 1_000)
  ...
  test "rejects clients above the configured total client cap" do
    ...
    assert {:ok, ^telegram} = :gen_tcp.recv(first, 0, 1_000)
    assert {:ok, ^telegram} = :gen_tcp.recv(second, 0, 1_000)
